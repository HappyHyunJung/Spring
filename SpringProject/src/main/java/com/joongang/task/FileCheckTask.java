...
public class FileCheckTask {
	...
	@Scheduled(cron = "0 0 0 ? * SUN")  // (cron = "0 * * ? * *")  일요일마다 upload  폴더 파일 삭제하는 코드
	public void checkFiles() throws Exception {
		...
		for(int i = 0; i < DayOfWeek.SUNDAY.getValue(); i++) {  // 이전에 동작했던 7일동안의 파일을 의미한다
			...
			uploadpath = sdf.format(cal.getTime()).replace('-', '\\'); // 저장된 파일 (2023\\월\\일)로 저장돼서 '\\'로 바꾼다
			log.info("cal...uploadpath : " + uploadpath);
			List<BoardAttachVO> fileList = attachMapper.getOldFiles(uploadpath);  // 
			
			//files on the tbl_attach - tbl_attach 파일 목록을 가져온다 (fileListPaths에 저장된다)
